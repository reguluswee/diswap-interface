...
// import useGetPoolLength from "../../hooks/nftpool/useGetPoolLength";

import useGetNftpoolInfos from "../../hooks/use-get-nftpoolInfo";
...
export default function MiningTable() {
    ...
    const { nftpoolCaculateInfo } = useGetNftpoolInfos(nftpoolContract,Number(chainId),web3);

    // const { nftpoolLength } =useGetPoolLength(nftpoolContract);
    // const [nftContract, setNftContract] = useState<any>(null);
    // const { nftsymbol } = useGetNftSymbol(nftContract);
    ...
    useEffect(() => {
        if (web3 && chainId) {
            ...
            const nftminerContractInstance = nftutils.getNFTPoolContract(chainId, web3);
            setNftpoolContract(nftminerContractInstance);
    ...
    async function goToNftFarm(pid: number,wnftaddress: string,nftaddress: string,mussesId:string){

        history.push(`/nftfarm/${type}/${pid}/${nftaddress}/${wnftaddress}/${mussesId}`)
    }
    ...
    const nftlistData = useMemo(() => {
        if (!nftpoolCaculateInfo) return [];
        return nftpoolCaculateInfo;
        // return nftpoolCaculateInfo.filter((item) => {
    ...
    function renderList() {
        ...
        if (isLp && !isNFT) {
            ...
            return nftlistData.map((item, index) => (
                <ItemContainer className="borderRow" key={index} onClick={() => goToNftFarm(Number(item.id),item.wnft,item.nft,item.mussesId)}>
                    {item.amount === 0 && (
                        <Tag
                            color="error"
                            style={{
                    ...
                    <HeaderRow>
                        <Row style={{ width: '35%' }}>
                            <Image src={item.logoURI} size={'50px'}/>
                        </Row>
                        <RowBetween style={{ width: '55%' }}>
                            <TextLabel>抵押总量:</TextLabel>
                            <TextValue>
                                <Value value={item.amount} /> NFT
                            </TextValue>
                        </RowBetween>
                    </HeaderRow>
                    <HeaderRow style={{ marginTop: '0.6rem' }}>
                        <RowBetween style={{ width: '35%' }}>
                            <OrangeText>{`${item.nftsymbol}`}</OrangeText>
                        </RowBetween>
                        <RowBetween style={{ width: '55%' }}>
                            <TextLabel>TVL:</TextLabel>
                            <TextValue>
    ...
    return (
        <AppBody style={{ marginTop: '1rem', background: 'transparent', boxShadow: 'none'}}>
            <Wrapper>
                ...
                <SearchWrapper>
                    <SearchButton className={(isLp && !isNFT) ? 'active' : ''} to={'/mining/lp'}>{t('lpTab')}</SearchButton>
                    <SearchButton className={(!isLp && !isNFT) ? 'active' : ''} to={'/mining/single'}>{t('singleCurrency')}</SearchButton>
                    <SearchButton className={isNFT ? 'active' : ''} to={'/mining/nft'}>{t('nftTab')}</SearchButton>
                    <SearchInputWrapper>
